-- 🎯 VERIFICAÇÃO FINAL CORRIGIDA - Execute no SQL Editor do Supabase Dashboard
...
-- 2. TOKENS ATIVOS PARA ESSES USUÁRIOS (CORRIGIDO)
SELECT 
  u.name,
  u.preferred_reminder_time,
...
FROM users u
LEFT JOIN push_subscriptions ps ON u.entra_id = ps.user_id
WHERE u.preferred_reminder_time = '14:15:00'
  AND u.reminder_frequency != 'disabled'
GROUP BY u.name, u.preferred_reminder_time
ORDER BY u.name;

-- 3. SIMULAR O QUE O SCHEDULER VAI FAZER (CORRIGIDO)
SELECT 
  u.name,
  u.preferred_reminder_time,
  ps.platform,
