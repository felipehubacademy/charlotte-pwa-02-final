-- ─────────────────────────────────────────────────────────────────────────────
-- 030_rn_achievement_rewards_via_practice.sql
--
-- Two changes:
-- 1. rn_on_practice_insert: skip XP milestone check for achievement_reward_*
--    practice types (prevents infinite loop / double-counting).
-- 2. rn_award_achievements: insert into rn_user_practices instead of direct
--    UPDATE on rn_user_progress — this lets the trigger handle total_xp and
--    leaderboard, and makes the bonus appear in "Hoje" + "Atividade Recente".
-- ─────────────────────────────────────────────────────────────────────────────

-- ── 1. Update trigger: skip milestones for achievement_reward_* ───────────────
...
BEGIN
  ...
  -- Skip XP milestone check for achievement reward practices (avoids recursion)
  IF NEW.practice_type NOT LIKE 'achievement_reward_%' THEN
    ...
    FOREACH v_milestone IN ARRAY v_milestones LOOP
      IF v_old_xp < v_milestone AND v_new_xp >= v_milestone THEN
        ...
        IF NOT EXISTS (
          ...
          -- Insert as practice so bonus appears in Hoje + Atividade Recente
          INSERT INTO public.rn_user_practices (user_id, practice_type, xp_earned)
            VALUES (NEW.user_id, 'achievement_reward_xp_' || v_milestone, v_ach_bonus);
        END IF;
      END IF;
    END LOOP;
...
-- ── 2. Update rn_award_achievements: use practice insert instead of direct UPDATE
...
BEGIN
  ...
  -- Macro to award: inserts achievement + practice row (trigger handles XP totals)
  -- Pattern repeated per achievement below.
  ...
  IF v_total_practices >= 1 AND NOT EXISTS (SELECT 1 FROM charlotte.user_achievements WHERE user_id = p_user_id::text AND achievement_type = v_code) THEN
    ...
    INSERT INTO public.rn_user_practices (user_id, practice_type, xp_earned) VALUES (p_user_id, 'achievement_reward_' || v_code, v_bonus);
