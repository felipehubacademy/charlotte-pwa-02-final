-- Migration: streak e achievements usam timezone do próprio usuário
-- Problema anterior: current_date e CURRENT_DATE retornam data UTC no PostgreSQL.
-- Para usuários fora de UTC, isso causava streak incorreto (ex: prática às 22h BRT
-- era registrada como dia seguinte UTC, fazendo dias distintos colidirem).
--
-- Fix: buscar charlotte.users.timezone e usar
--   (current_timestamp AT TIME ZONE v_user_tz)::date
-- em todos os lugares que antes usavam current_date.

-- ── 1. rn_award_achievements: usa timezone do usuário ────────────────────────
...
DECLARE
  ...
  v_learn_count     int     := 0;
  v_today_xp        int     := 0;
  v_user_tz         text    := 'America/Sao_Paulo';
  v_local_date      date;
  v_current_hour    int;
...
BEGIN
  ...
  -- Timezone do usuário (fallback: Sao_Paulo para quem ainda não sincronizou)
  SELECT COALESCE(NULLIF(timezone, ''), 'America/Sao_Paulo')
    INTO v_user_tz
    FROM charlotte.users WHERE id = p_user_id;

  -- Data e hora locais do usuário
  v_local_date   := (current_timestamp AT TIME ZONE v_user_tz)::date;
  v_current_hour := EXTRACT(HOUR FROM current_timestamp AT TIME ZONE v_user_tz)::int;
  ...
  -- XP de hoje no fuso do usuário
  SELECT COALESCE(SUM(xp_earned), 0) INTO v_today_xp
    FROM charlotte.practices WHERE user_id = p_user_id
      AND (created_at AT TIME ZONE v_user_tz)::date = v_local_date
      AND practice_type NOT LIKE 'achievement_reward_%'
      AND practice_type NOT LIKE 'mission_reward_%';
  ...
  LOOP
    ...
    -- Achievements diários podem se repetir a cada dia (no fuso do usuário)
    IF ach.requirement_type IN ('today_xp', 'early_bird', 'night_owl') THEN
      CONTINUE WHEN EXISTS (
        SELECT 1 FROM charlotte.user_achievements
        WHERE user_id = p_user_id::text
          AND achievement_id = ach.id
          AND (earned_at AT TIME ZONE v_user_tz)::date = v_local_date
      );
    END IF;
...
-- ── 2. rn_on_practice_insert: streak usa data local do usuário ───────────────
...
DECLARE
  ...
  v_ach_bonus    int;
BEGIN
  -- Timezone do usuário (deve existir em charlotte.users após migration 20260420)
  SELECT COALESCE(NULLIF(timezone, ''), 'America/Sao_Paulo')
    INTO v_user_tz
    FROM charlotte.users WHERE id = NEW.user_id;

  -- "Hoje" no fuso do usuário, não em UTC
  v_today := (current_timestamp AT TIME ZONE v_user_tz)::date;

  -- Atualiza XP + streak usando data local do usuário
  ...
  -- XP milestones (apenas práticas reais)
